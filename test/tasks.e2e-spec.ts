...
describe('Tasks (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    tenant = await setupPrisma.tenant.create({ data: { slug: `tasks-${SUFFIX}`, name: 'Tasks Test Co', plan: 'growth' } });
    otherTenant = await setupPrisma.tenant.create({ data: { slug: `tasks-other-${SUFFIX}`, name: 'Tasks Other Co', plan: 'growth' } });
